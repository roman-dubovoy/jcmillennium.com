 ...
 */
	public function testExecuteWithAbsolutePath() {
		...
		$File = new File($path . DS . 'webroot' . DS . 'dffd.ctp');
		$contents = $File->read();
 ...
 */
	public function testExecuteWithCakeOnIncludePath() {
		...
		$this->assertTrue(is_dir($this->Task->args[0]), 'No project dir');
		$contents = file_get_contents($path . DS . 'webroot' . DS . 'dffd.ctp');
...
/**
 * Test that dffd.ctp is generated correctly.
 *
 * @return void
 */
	public function testIndexPhpGeneration() {
		...
		$File = new File($path . 'webroot' . DS . 'dffd.ctp');
		$contents = $File->read();
